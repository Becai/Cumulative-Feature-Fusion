...
# CFF parameters
parser.add_argument('--cff-stage', default=[1, 2, 3, 4], nargs='+', type=int)
parser.add_argument('--cff-loss-weight', default=1, type=float)
parser.add_argument('--cff-temperature', default=1, type=float)
parser.add_argument('--cff-decay-func', default='cos', type=str)  # linear x2 cos
# parser.add_argument('--cff-loss-type', default='soft', type=str)  # hard soft
...
def main():
    ...
    teacher = None
    if args.teacher:
        teacher = create_model(  # teacher model
            args.teacher,
            num_classes=args.num_classes)
    ...
    if args.dataset == 'cifar100':
        transform = Compose([
            Resize((224, 224)),  # 调整图像大小以匹配预训练模型的输入
            ToTensor(),  # 将 PIL 图像转换为张量
        ])
